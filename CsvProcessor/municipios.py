...
import Config as c
#https://www.youtube.com/watch?v=vYVVjJvy2Os&list=RDGMEM2VCIgaiSqOfVzBAjPJm-ag&index=9


entrada = c.municipios
...
with open(entrada,"r", encoding = "latin1") as arquivo:
    for linha in arquivo:
        ...
        if len(buffer) >= chunkSize:
            ...
            sql = f"INSERT INTO Municipios ({', '.join(colunas)}) VALUES ({placeHolders})"


            try:
                cursor.executemany(sql,data)
                con.commit()
                print("DEUUUU GREEENNNNNNNNNNNN")
